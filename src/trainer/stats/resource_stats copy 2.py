...
class ResourceMonitoringStats(TrainerStats):
    ...
    def _save_stats(self):
        ...
        csv_row = (f"{s['step']},{s['epoch']},{s.get('step_time_sec', 0):.4f},"
                  ...
                  f"{s.get('forward_gpu_mem_mb', 0):.2f},{s.get('backward_gpu_mem_mb', 0):.2f},{s.get('optim_gpu_mem_mb', 0):.2f},"
                  f"{s.get('forward_time_sec', 0)*1000:.2f},{s.get('backward_time_sec', 0)*1000:.2f},{s.get('optimizer_time_sec', 0)*1000:.2f}\n")
                ...
                if not csv_file.exists():
                    with open(csv_file, 'w') as f:
                        f.write("step,epoch,step_time_sec,io_time_sec,samples_per_sec,loss,"
                               ...
                               "forward_gpu_mem_mb,backward_gpu_mem_mb,optim_gpu_mem_mb,"
                               "forward_time_ms,backward_time_ms,optim_time_ms\n")
    ...
    def _generate_plots(self):
        ...
        try:
            ...
            if gpu_mem:
                ...
                max_mem = max(gpu_mem)
                axes[0, 1].set_ylim(0, max_mem * 1.1)
            ...
            # Generate phase plot
            self._generate_phase_plots()
            
        except Exception as e:
            print(f"[ResourceStats] Warning: Could not generate plots: {e}")
    
    def _generate_phase_plots(self):
        """Generate phase-based plots: GPU Memory and Phase Duration."""
        try:
            import matplotlib
            matplotlib.use('Agg')
            import matplotlib.pyplot as plt
            ...
            # GPU Memory by phase (GB)
            gpu_mem_step = [s.get('gpu_memory_used_mb', 0) / 1024 for s in self.stats_history]
            gpu_mem_fwd  = [s.get('forward_gpu_mem_mb', 0) / 1024 for s in self.stats_history]
            gpu_mem_bwd  = [s.get('backward_gpu_mem_mb', 0) / 1024 for s in self.stats_history]
            gpu_mem_opt  = [s.get('optim_gpu_mem_mb', 0) / 1024 for s in self.stats_history]

            # Phase Duration (ms)
            time_step_ms = [s.get('step_time_sec', 0) * 1000 for s in self.stats_history]
            time_fwd_ms  = [s.get('forward_time_sec', 0) * 1000 for s in self.stats_history]
            time_bwd_ms  = [s.get('backward_time_sec', 0) * 1000 for s in self.stats_history]
            time_opt_ms  = [s.get('optimizer_time_sec', 0) * 1000 for s in self.stats_history]

            colors = {'step': 'blue', 'forward': 'orange', 'backward': 'green', 'optim': 'red'}
            lw = 1

            fig, axes = plt.subplots(1, 2, figsize=(14, 6))
            fig.suptitle('ResNet152 Training Summary by Phase', fontsize=16, fontweight='bold')

            # Plot 1: GPU Memory by phase
            axes[0].plot(steps, gpu_mem_step, color=colors['step'], lw=lw, alpha=0.7, label='step')
            axes[0].plot(steps, gpu_mem_fwd,  color=colors['forward'], lw=lw, alpha=0.7, label='forward')
            axes[0].plot(steps, gpu_mem_bwd,  color=colors['backward'], lw=lw, alpha=0.7, label='backward')
            axes[0].plot(steps, gpu_mem_opt,  color=colors['optim'], lw=lw, alpha=0.7, label='optim')
            axes[0].set_title('GPU Memory Used (GB)')
            ...
            axes[1].plot(steps, time_step_ms, color=colors['step'], lw=lw, alpha=0.7, label='step')
            axes[1].plot(steps, time_fwd_ms,  color=colors['forward'], lw=lw, alpha=0.7, label='forward')
            axes[1].plot(steps, time_bwd_ms,  color=colors['backward'], lw=lw, alpha=0.7, label='backward')
            axes[1].plot(steps, time_opt_ms,  color=colors['optim'], lw=lw, alpha=0.7, label='optim')
            axes[1].set_title('Phase Duration (ms)')
            ...
            output_file = self.output_dir / "training_metrics_phase.png"
            plt.savefig(output_file, dpi=150, bbox_inches='tight')
            plt.close()

            print(f"[ResourceStats] Phase visualization saved to {output_file}")

        except Exception as e:
            print(f"[ResourceStats] Warning: Could not generate phase plots: {e}")
    ...
    def stop_step(self):
        ...
        # Store every 10 steps to keep full history without memory bloat
    ...
    def stop_forward(self):
        ...
        if self.forward_start_time:
            ...
            self.current_stats['forward_gpu_mem_mb'] = torch.cuda.memory_allocated() / 1024**2 if torch.cuda.is_available() else 0
    ...
    def stop_backward(self):
        ...
        if self.backward_start_time:
            ...
            self.current_stats['backward_gpu_mem_mb'] = torch.cuda.memory_allocated() / 1024**2 if torch.cuda.is_available() else 0
    ...
    def stop_optimizer_step(self):
        ...
        if self.optimizer_start_time:
            ...
            self.current_stats['optim_gpu_mem_mb'] = torch.cuda.memory_allocated() / 1024**2 if torch.cuda.is_available() else 0
